...
# color list for Mahsa's plot

colors = c("#CE2626", "#650D0D", "##B35F5F", "#420F2A")
app$callback(
  ...
  function(ratings_range,year_range) {
    ...
    plot  <- ggplot(df, aes(x = release_year, y = count, color = rating)) +
      ...
      # scale_color_manual(values=c("#CE2626", "#650D0D", "##B35F5F", "#420F2A"))
...
```{r}
 plot  <- ggplot(data ,aes(x = release_year, y = count, color = rating)) +
      geom_line()+      
      scale_size(range = c(2, 12)) +
      ggtitle('Movie rating in Netflix in different years') +
      labs(x = 'Years', y= "Number of movie") +
